...
def get_args():
    ...
    p.add_argument(
        ...
        type=int,
        default=16,
        help="Forward batch size (raise if GPU memory allows; main cost is W2V-BERT forward)",
    )
    p.add_argument(
        "--cudnn-benchmark",
        default=True,
    ...
    p.add_argument(
        "--no-gpu-mel-frontend",
        action="store_true",
        help="Use HuggingFace CPU numpy feature extractor (slow; for debugging)",
    )
    p.add_argument(
        "--no-l2-normalize",
        action="store_true",
...
def main():
    ...
    use_gpu_mel = not args.no_gpu_mel_frontend
    if use_gpu_mel:
        print("Log-mel: GPU/torch frontend (SeamlessM4T-compatible).", file=sys.stderr)
    else:
        print("Log-mel: HF CPU AutoFeatureExtractor (--no-gpu-mel-frontend).", file=sys.stderr)
