...
sad_type="oracle"       # oracle / system (Silero) / pyannote / funasr_fsmn
...
Env: USM_FTCODE (default: ./wpt_mhfa_zl389 vendored code), WPT_MHFA_CKPT_DIR (default: \$HOME/Encode-explore/USM_FTcode/ckpt_asv/..._h8_c128_fixed24000),
     ...
     W2VBERT_EMB_DEVICE, EMB_BATCH_SIZE, EMB_NJ, HF_TOKEN (PyAnnote), DEMUCS_DEVICE, PYANNOTE_DEVICE, OVERLAP_DEVICE,
...
Note: Stage 6 uses vendored model code; checkpoints are user-supplied (not zl389 model_base_*.pth).
      Tune embedding speed: EMB_BATCH_SIZE (default 16), EMB_NJ (default 1; multi-GPU only)."
