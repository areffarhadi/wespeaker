...
#               2026 — WPT + W2V-BERT-2.0 + MHFA (vendored wpt_mhfa_zl389/)
#
...
window_secs=1.5
period_secs=0.75
# Set true to force slow HF CPU feature extractor (debug only).
no_gpu_mel_frontend=false

. tools/parse_options.sh

[ -z "${ckpt_dir}" ] && echo "$0: set --ckpt-dir (args.json + best_model.pt)" && exit 1
if [ -z "${usm_ftcode}" ]; then
    _v2="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")/.." && pwd)"
    usm_ftcode="${_v2}/wpt_mhfa_zl389"
fi
if [ ! -d "${usm_ftcode}" ] || [ ! -f "${usm_ftcode}/main_train_simple_sv_wpt_w2vbert_mhfa_zl389.py" ]; then
    echo "$0: vendored backend missing under ${usm_ftcode} (set --usm-ftcode)" >&2
    exit 1
fi
...
# WeSpeaker diarization needs kaldiio (in repo .venv). USM enc-env often lacks it.
...
for scp_subfile in "${split_files[@]}"; do
    ...
    _cmd=(
        "${extract_python}" wespeaker/diar/extract_emb_w2vbert_wpt_mhfa_zl389.py
        --scp "${scp_subfile}"
        --segments "${segments}"
        --ark-path "${write_ark}"
        --ckpt-dir "${ckpt_dir}"
    ...
    if [ "$no_gpu_mel_frontend" = true ]; then
        _cmd+=(--no-gpu-mel-frontend)
    fi
...
if [ ${#pids[@]} -gt 0 ]; then
    for i in "${!pids[@]}"; do
        if ! wait "${pids[$i]}"; then
            echo "$0: extract_emb_w2vbert_wpt_mhfa_zl389.py failed (pid ${pids[$i]}). Log: ${logfs[$i]}" >&2
            echo "----- tail ${logfs[$i]} -----" >&2
            tail -n 80 "${logfs[$i]}" >&2
            exit 1
        fi
...
echo "Finish extract embedding (WPT+MHFA zl389). (${emb_lines} lines in emb.scp)"
